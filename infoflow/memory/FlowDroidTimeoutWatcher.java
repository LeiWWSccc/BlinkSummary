...
public class FlowDroidTimeoutWatcher implements IMemoryBoundedSolverStatusNotification {
	 ...
	 * @param solver A solver that shall be terminated when the timeout is
	 * reached
	 */
	...
	public void start() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (!stopped && (System.currentTimeMillis() - startTime) < 1000 * timeout) {
				...
				// If things have not stopped on their own account, we force them to
